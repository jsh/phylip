...
void   arbitree(void);
void   yourtree(void);
void   initmovenode(tree *, node **, long, long, long *, long *, initops, pointarray, Char *, Char *, FILE *);
void   buildtree(void);
void   rearrange(void);
...
tree * curtree;
long outgrno, screenlines, col, treelines, leftedge, topedge, vmargin, hscroll, vscroll, scrollinc, screenwidth, farthest, nonodes = 0;
/* outgrno indicates outgroup */
boolean weights, thresh, outgropt, ancvar, questions, allsokal, allwagner, mixture, factors, noroot,  waswritten;
boolean *ancone, *anczero, *ancone0, *anczero0;
Char *factor;
double threshold;
double *threshwt;
bitptr wagner, wagner0;
unsigned char che[(long)question - (long)horiz + 1];
...
long dispchar, dispword, dispbit, atwhat, what, fromwhere, towhere, oldoutgrno, compatible;
...
void inputoptions(void)
{
  ...
  for (i = 0; i < chars; i++)
  {
    if (weight[i] > 0)
    {
      noroot = (noroot && ancone[i] && anczero[i] && ((((1L << (i % bits + 1)) & wagner[i / bits]) != 0) || threshold <= 2.0));
    }
...
void doinput(void)
{
  ...
  allocrest();
  inputoptions();
  inputdata(curtree->nodep, true, false, stdout);
}  /* doinput */
...
void reroot(node *outgroup)
{
  ...
  onleft = (p == curtree->root->next);
  if (restoring)
    if (!onleft && wasleft)
    {
      p = curtree->root->next->next;
      q = curtree->root->next;
    }
    else
    {
  ...
  {
    if (onleft)
      oldoutgrno = curtree->root->next->next->back->index;
    else
      oldoutgrno = curtree->root->next->back->index;
    wasleft = onleft;
    p = curtree->root->next;
    q = curtree->root->next->next;
  }
  ...
  if (restoring)
  {
    if (!onleft && wasleft)
    {
      outgroup->back->back = curtree->root->next;
      outgroup->back = curtree->root->next->next;
    }
    else
    {
...
void move_hyptrav(node *r)
{
  ...
  {
    memcpy(zerobelow->bits_, curtree->nodep[r->back->index - 1]->statezero, words * sizeof(long));
    memcpy(onebelow->bits_, curtree->nodep[r->back->index - 1]->stateone, words * sizeof(long));
  }
  for (i = 0; i < words; i++)
  {
    s0 = r->statezero[i];
...
void move_hypstates(void)
{
  ...
  move_filltrav(curtree->root);
  move_hyptrav(curtree->root);
}  /* move_hypstates */
...
void move_drawline(long i, long lastline)
{
  ...
  (void)lastline;                       // RSGnote: Parameter never used.
...
void arbitree(void)
{
  ...
  add2(curtree->nodep[0], curtree->nodep[1], curtree->nodep[spp], &curtree->root, restoring, wasleft, curtree->nodep);
  for (i = 3; i <= spp; i++)
    add2(curtree->nodep[spp+ i - 3], curtree->nodep[i - 1], curtree->nodep[spp + i - 2], &curtree->root, restoring, wasleft, curtree->nodep);
  for (i = 0; i < nonodes; i++)
    in_tree[i] = true;
}  /* arbitree */
...
void yourtree(void)
{
  ...
  do {
    ...
    add2(curtree->nodep[j - 1], curtree->nodep[i - 1], curtree->nodep[spp + i - 2], &curtree->root, restoring, wasleft, curtree->nodep);
  } while (i != spp);
  for (i = 0; i < nonodes; i++)
    in_tree[i] = true;
}  /* yourtree */
...
void initmovenode(tree *treep, node **p, long len, long nodei, long *ntips, long *parens, initops whichinit, pointarray treenode, Char *str, Char *ch, FILE *intree)
{
  /* initializes a node */
...
void get_usertree(void)
{
  ...
  nextnode = 0;
  haslengths = 0;
  treeread(curtree, intree, &curtree->root, curtree->nodep, &goteof, &firsttree, &nextnode, &haslengths, initmovenode, false, nonodes);
...
void buildtree(void)
{
  ...
  if (outgropt && in_tree[outgrno - 1])
    reroot(curtree->nodep[outgrno - 1]);
...
void rearrange(void)
{
  ...
  inpnum(&i, &ok1);
  ok1 = (ok1 && i >= 1 && i < spp * 2 && i != curtree->root->index);
  if (ok1)
  {
    ...
    if (ok2)
    {
      ok2 = (curtree->nodep[j - 1] != curtree->nodep[curtree->nodep[i - 1]->back->index - 1]);
      p = curtree->nodep[j - 1];
      ...
      if (ok1 && ok2)
      {
        ...
        towhere = j;
        re_move2(&curtree->nodep[i - 1], &q, &curtree->root, &wasleft, curtree->nodep);
        add2(curtree->nodep[j - 1], curtree->nodep[i - 1], q, &curtree->root, restoring, wasleft, curtree->nodep);
      }
      lastop = rearr;
    }
  }
...
void tryadd(node *p, node *item, node *nufork, double *place)
{
  ...
  add2(p, item, nufork, &curtree->root, restoring, wasleft, curtree->nodep);
  evaluate(curtree->root);
  place[p->index - 1] = -like;
  re_move2(&item, &nufork, &curtree->root, &wasleft, curtree->nodep);
}  /* tryadd */
...
void try(void)
{
  ...
  if (q->next->back->index == i)
    ...
    fromwhere = q->next->back->index;
  rute = curtree->root;
  if (curtree->root->index == curtree->nodep[i - 1]->back->index)
  {
  ...
  re_move2(&curtree->nodep[i - 1], &dummy, &curtree->root, &wasleft, curtree->nodep);
  oldleft = wasleft;
  curtree->root = rute;
  addpreorder(curtree->root, curtree->nodep[i - 1], dummy, place);
  wasleft =oldleft;
  restoring = true;
  add2(curtree->nodep[fromwhere - 1], curtree->nodep[what - 1], dummy, &curtree->root, restoring, wasleft, curtree->nodep);
  like = -current;
...
void undo(void)
{
  ...
  switch (lastop)
  {
    case rearr:
      restoring = true;
      oldleft = wasleft;
      re_move2(&curtree->nodep[what - 1], &q, &curtree->root, &wasleft, curtree->nodep);
      btemp = wasleft;
      wasleft = oldleft;
      add2(curtree->nodep[fromwhere - 1], curtree->nodep[what - 1], q, &curtree->root, restoring, wasleft, curtree->nodep);
      wasleft = btemp;
      restoring = false;
      temp = fromwhere;
      fromwhere = towhere;
    ...
    case flipp:
      q = curtree->nodep[atwhat - 1]->next->back;
      curtree->nodep[atwhat - 1]->next->back = curtree->nodep[atwhat - 1]->next->next->back;
      curtree->nodep[atwhat - 1]->next->next->back = q;
      curtree->nodep[atwhat - 1]->next->back->back = curtree->nodep[atwhat - 1]->next;
      curtree->nodep[atwhat - 1]->next->next->back->back = curtree->nodep[atwhat - 1]->next->next;
      break;
...
void flip(void)
{
  ...
  if (ok)
  {
    p = curtree->nodep[i - 1]->next->back;
    curtree->nodep[i - 1]->next->back = curtree->nodep[i - 1]->next->next->back;
    curtree->nodep[i - 1]->next->next->back = p;
    curtree->nodep[i - 1]->next->back->back = curtree->nodep[i - 1]->next;
    curtree->nodep[i - 1]->next->next->back->back = curtree->nodep[i - 1]->next->next;
    atwhat = i;
    lastop = flipp;
  }
  move_printree();
...
void changeoutgroup(void)
{
  ...
  do {
    ...
    inpnum(&i, &ok);
    ok = (ok && in_tree[i - 1] && i >= 1 && i <= nonodes && i != curtree->root->index);
    if (ok)
      outgrno = i;
  } while (!ok);
