...
void inputdata(pointptr treenode, boolean dollo, boolean printdata, FILE *outfile)
{
  ...
  for (i = 1; i <= spp; i++)
  {
    ...
    for (j = 1; j <= chars; j++)
    {
      ...
      if (charstate == '1')
        treenode[i - 1]->stateone[l - 1] = ((long)treenode[i - 1]->stateone[l - 1]) | (1L << k);
      if (charstate == '0')
        treenode[i - 1]->statezero[l - 1] = ((long)treenode[i - 1]->statezero[l - 1]) | (1L << k);
...
void add(node *below, node *newtip, node *newfork, node **root, pointptr treenode)
{
...
void add2(node *below, node *newtip, node *newfork, node **root, boolean restoring, boolean wasleft, pointptr treenode)
{
  /* inserts the nodes newfork and its left descendant, newtip,
     to the tree.   below becomes newfork's right descendant */
...
void add3(node2 *below, node2 *newtip, node2 *newfork, node2 **root, pointptr2 treenode)
{
  /* inserts the nodes newfork and its left descendant, newtip,
     to the tree.  below becomes newfork's right descendant.
     The global variable root is also updated */
...
void re_move(node **item, node **forknode, node **root, pointptr treenode)
{
  /* removes nodes item and its ancestor, forknode, from the tree.
     the new descendant of forknode's ancestor is made to be
     forknode's second descendant (other than item).  Also
     returns pointers to the deleted nodes, item and forknode.
     The global variable root is also updated */
  /* used in dollop & dolpenny */
  node *p, *q;
  ...
  *forknode = treenode[(*item)->back->index - 1];

  if (*root == *forknode)
  {
    if (*item == (*forknode)->next->back)
      *root = (*forknode)->next->next->back;
    else
      *root = (*forknode)->next->back;
  }
...
void re_move2(node **item, node **forknode, node **root, boolean *wasleft, pointptr treenode)
{
  /* removes nodes item and its ancestor, forknode, from the tree.
     the new descendant of forknode's ancestor is made to be
     forknode's second descendant (other than item).   Also
...
void re_move3(node2 **item, node2 **forknode, node2 **root, pointptr2 treenode)
{
  /* removes nodes item and its ancestor, forknode, from the tree.
     the new descendant of forknode's ancestor is made to be
     forknode's second descendant (other than item).  Also
...
void standev(long numtrees, long minwhich, double minsteps, double *nsteps, double **fsteps, longer seed)
    ...
    {           /* Shimodaira-Hasegawa test using normal approximation */
      ...
      for (i = 0; i < numtrees; i++)    /* compute covariances of trees */
      {
        ...
        for (j = 0; j <=i; j++)
        {
          ...
          {
            if (weight[k] > 0)
              temp = temp + weight[k]*(fsteps[i][k]/-sum)
                *(fsteps[j][k]/-sum2);
          }
          covar[i][j] = temp;
          if (i != j)
