...
void   arbitree(void);
void   yourtree(void);
void   initdolmovenode(tree *, node **, long, long, long *, long *, initops, pointarray, Char *, Char *, FILE *);
void   buildtree(void);
void   rearrange(void);
...
long outgrno, col, screenlines, screenwidth, scrollinc, treelines, leftedge, topedge, vmargin, hscroll, vscroll, farthest, nonodes = 0;
/* outgrno indicates outgroup */
boolean weights, thresh, ancvar, questions, dollo, factors, waswritten;
...
{  /* get final states for intermediate nodes */
  ...
  for (i = 0; i < words; i++) {
    if (p->back == NULL) {
      ...
      s0 = curtree->nodep[p->back->index - 1]->statezero[i];
      s1 = curtree->nodep[p->back->index - 1]->stateone[i];
    }
    z0 = (s0 & p->statezero[i]) | (p->next->back->statezero[i] & p->next->next->back->statezero[i]);
    z1 = (s1 & p->stateone[i]) | (p->next->back->stateone[i] & p->next->next->back->stateone[i]);
    if (dollo) {
...
void dolmove_count(node *p)
{
  ...
  if (dollo) {
    for (i = 0; i < words; i++)
      steps[i] = (curtree->nodep[p->back->index - 1]->stateone[i] &
                  p->statezero[i] & zeroanc[i]) |
                 (curtree->nodep[p->back->index - 1]->statezero[i] &
                  p->stateone[i] & oneanc[i]);
  } else {
    for (i = 0; i < words; i++)
      steps[i] = curtree->nodep[p->back->index - 1]->stateone[i] &
                 curtree->nodep[p->back->index - 1]->statezero[i] & p->stateone[i] &
                 p->statezero[i];
  }
  j = 1;
  l = 0;
...
void dolmove_hypstates(void)
{
  ...
  filltrav(curtree->root);
  dolmove_hyptrav(curtree->root);
}  /* dolmove_hypstates */
...
void arbitree(void)
{
  ...
  for (i = 3; i <= spp; i++)
    add2(curtree->nodep[spp + i - 3], curtree->nodep[i - 1], curtree->nodep[spp + i - 2], &(curtree->root),
      restoring, wasleft, curtree->nodep);
  for (i = 0; i < nonodes; i++)
    in_tree[i] = true;
}  /* arbitree */
...
void yourtree(void)
{
  ...
  do {
    ...
    add2(curtree->nodep[j - 1], curtree->nodep[i - 1], curtree->nodep[spp + i - 2], &(curtree->root),
         restoring, wasleft, curtree->nodep);
  } while (i != spp);
  for (i = 0; i < nonodes; i++)
    in_tree[i] = true;
...
void initdolmovenode(tree * treep, node **p, long len, long nodei,
...
void buildtree(void)
{
  ...
  switch (how)
  {
    ...
    case use:
      ...
      nextnode = 0;                           /**/
      haslengths = 0;                         /**/
      treeread(curtree, intree, &(curtree->root), curtree->nodep, &goteof, &firsttree,
               &nextnode, &haslengths, initdolmovenode, false, nonodes);
...
void try(void)
{
  ...
  wasleft = oldleft;
  restoring = true;
  add2(curtree->nodep[fromwhere - 1], curtree->nodep[what - 1], dummy, &(curtree->root),
       restoring, wasleft, curtree->nodep);
  like = -current;
...
void undo(void)
{
  ...
  switch (lastop)
  {
    case rearr:
      ...
      btemp = wasleft;
      wasleft = oldleft;
      add2(curtree->nodep[fromwhere - 1], curtree->nodep[what - 1], q, &(curtree->root),
           restoring, wasleft, curtree->nodep);
      wasleft = btemp;
      restoring = false;
      temp = fromwhere;
    ...
    case flipp:
      ...
      curtree->nodep[atwhat - 1]->next->back =
        curtree->nodep[atwhat - 1]->next->next->back;
      curtree->nodep[atwhat - 1]->next->next->back = q;
      curtree->nodep[atwhat - 1]->next->back->back = curtree->nodep[atwhat - 1]->next;
      curtree->nodep[atwhat - 1]->next->next->back->back =
        curtree->nodep[atwhat - 1]->next->next;
      break;
  ...
  do {
    ...
    if (strchr("RSWH#.O?+TFX-UCQHJKL", ch) != NULL)
    {
      switch (ch)
      {
        ...
        case '#':
          nextinc(&dispchar, &dispword, &dispbit, chars, bits, &display, numsteps, weight);
          dolmove_printree();
          break;
